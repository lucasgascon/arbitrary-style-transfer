...
vgg_layers_styleloss = {'relu1_1': [0,1], 'relu2_1': [2,3,4,5,6], 'relu3_1': [7,8,9,10,11], 'relu4_1': [12,13,14,15,16,17,18,19,20]}
        ...
        for i, layer in enumerate(original_features):
            if i in vgg_layers_styleloss['relu1_1']:
                if isinstance(layer, nn.ReLU) and layer.inplace:
                    ...
                    self.encoder_1.add_module("relu1_1", nn.ReLU(inplace=False))
                else:
            ...
            if i in vgg_layers_styleloss['relu2_1']:
                if isinstance(layer, nn.ReLU) and layer.inplace:
                    # If the layer is a ReLU with inplace=True, replace it with a ReLU with inplace=False
                    self.encoder_2.add_module("relu2_1", nn.ReLU(inplace=False))
                else:
                    # Otherwise, just add the original layer
            ...
            if i in vgg_layers_styleloss['relu3_1']:
                if isinstance(layer, nn.ReLU) and layer.inplace:
                    # If the layer is a ReLU with inplace=True, replace it with a ReLU with inplace=False
                    self.encoder_3.add_module("relu3_1", nn.ReLU(inplace=False))
                else:
                    # Otherwise, just add the original layer
            ...
            if i in vgg_layers_styleloss['relu4_1']:
                if isinstance(layer, nn.ReLU) and layer.inplace:
                    # If the layer is a ReLU with inplace=True, replace it with a ReLU with inplace=False
                    self.encoder_4.add_module("relu4_1", nn.ReLU(inplace=False))
                else:
                    # Otherwise, just add the original layer
...
class StyleTransferNet(nn.Module):
    ...
    def forward(self, content, style):
        ...
        g_t = self.decoder.decoder_4(t) # Input is 512 channels and output is 256 channels
        
        if self.skip_connections:
            g_t = g_t + content_3
            
        g_t = self.decoder.decoder_3(g_t) # Input is 256 channels and output is 128 channels
        
        if self.skip_connections:
            g_t = g_t + content_2
            
        g_t = self.decoder.decoder_2(g_t) # Input is 128 channels and output is 64 channels
        
        if self.skip_connections:
            g_t = g_t + content_1
            
        g_t = self.decoder.decoder_1(g_t) # Input is 64 channels and output is 3 channels
